                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body p-4">
                                <form class="row g-3" action="{{ route('store.role.permission') }}" id="myForm"
                                    ...
                                    @foreach ($permission_groups as $group)
                                        <div class="row">
                                            ...
                                            <div class="col-9">
                                                @php
                                                    $user_permissions = App\Models\User::getPermissionByGroupName(
                                                        $group->group_name,
                                                    );
                                                @endphp
                                                @foreach ($user_permissions as $user_permission)
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="checkbox" name="permission[]"
                                                            id="flexCheckDefault{{ $user_permission->id }}"
                                                            value="{{ $user_permission->id }}">
                                                        <label class="form-check-label"
                                                            for="flexCheckDefault{{ $user_permission->id }}">{{ $user_permission->name }}
                                                        </label>
                                                    </div>
                                                @endforeach
                                                <br>
